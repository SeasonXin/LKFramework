...
CREATE TABLE `T_SYS_USER_LOGIN` (
  ...
  PRIMARY KEY (`ID`),
  KEY `I_USER_LOGIN__USING_STATUS` (`USING_STATUS`),
  KEY `I_USER_LOGIN__INSERT_TIME` (`INSERT_TIME`),
  KEY `I_USER_LOGIN__BUS_ID` (`BUS_ID`),
  KEY `I_USER_LOGIN__USER_ID` (`USER_ID`),
  KEY `I_USER_LOGIN__CELLPHONE` (`CELLPHONE`),
  KEY `I_USER_LOGIN__EMAIL` (`EMAIL`),
  KEY `I_USER_LOGIN__USER_CARD` (`USER_CARD`),
  KEY `I_USER_LOGIN__LOGIN_NAME` (`LOGIN_NAME`),
  KEY `I_USER_LOGIN__SYSTEM_TAG` (`SYSTEM_TAG`),
  KEY `I_USER_LOGIN__TOKEN` (`TOKEN`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='人员登录表';
